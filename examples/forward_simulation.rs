    ...
    pub fn new(
        ...
        PopulationParams {
            ...
            breakpoint: match xovers.partial_cmp(&0.0) {
                Some(std::cmp::Ordering::Greater) => {
                    Some(Exp::new(xovers / genome_length.value() as f64).unwrap())
                }
                Some(_) => None,
                None => None,
            },
...
fn mutate_tables(
    ...
    for i in 0..num_edges {
        ...
        let mut pos = e.left.value() + (rng.sample(exp) as forrustts::PositionLLType) + 1;
    ...
    if !simplified && actual_simplification_interval != -1 {
        ...
        for (i, h) in pop.edge_buffer.head_iter().rev().enumerate() {
            if *h != forrustts::nested_forward_list::NULL_INDEX {
                ...
                {
                    assert!((seg.node.value() as usize) < pop.tables.num_nodes());
                    edges.push(forrustts::Edge {
                        left: seg.left,
                        right: seg.right,
                        parent: head.try_into().unwrap(),
                        child: seg.node,
                    });
                }
            }
