...
/// let tsk_tables = forrustts_tables_trees::tskit_tools::convert_to_tskit_minimal(
///     &tables,
///     &is_sample,
///     forrustts_tables_trees::tskit_tools::simple_time_reverser(1_f64.into()),
///     true,
/// );
...
/// tables.add_edge(0, 100, 0, 1).unwrap(); // Add an edge
/// let is_sample = vec![0, 1]; // Mark the child node as a sample.
/// let tsk_tables = forrustts_tables_trees::tskit_tools::convert_to_tskit_and_drain_minimal(
///     &is_sample,
///     forrustts_tables_trees::tskit_tools::simple_time_reverser(1_f64.into()),
///     true,
///     &mut tables,
...
mod tests {
    ...
    fn test_convert_to_tskit() {
        ...
        tables.add_edge(0, 100, 0, 1).unwrap(); // Add an edge
        let is_sample = vec![0, 1]; // Mark the child node as a sample.
        let tsk_tables = convert_to_tskit_minimal(
            &tables,
            &is_sample,
            simple_time_reverser(1_f64.into()),
            true,
        );
        assert_eq!(tsk_tables.nodes().num_rows(), 2);
        assert_eq!(tsk_tables.edges().num_rows(), 1);
